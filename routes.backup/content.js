...
// Buscar dados de uma landing page específica por slug (pública)
...
// Atualizar dados de uma landing page específica por slug (PROTEGIDO)
router.put("/landing/:slug", authMiddleware, (req, res) => {
  try {
    const { slug } = req.params;
...
// Listar todas as landing pages (pública)
...
// Deletar uma landing page específica por slug (PROTEGIDO)
router.delete("/landing/:slug", authMiddleware, (req, res) => {
  try {
    const { slug } = req.params;
    const landingData = readData("landing_pages");
